...
# ------------------- DRIVE FOLDER -------------------
DRIVE_FOLDER_ID = "1u9uDdhJ0Q8GolChkIk6-otOi6pYggTxP"  # Your shared folder ID
...
def est_percentile(value, curve):
    ...
    v0,v1,p0,p1 = values[j-1],values[j-1],percs[j-1],percs[j]
...
def get_ai_recommendations(status, age_m, wfh_p, hfa_p, bmi):
    ...
    if status in ["Obese","Overweight"]:
        ...
        recs += [
            "- Iron, zinc, vitamin A-rich foods.",
...
if generate_button and growth_model and scaler:
    ...
    if report:
        ...
        st.markdown(f"[Open PDF in Drive]({drive_link})")
