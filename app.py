...
def upload_pdf_to_drive(pdf_buffer: BytesIO, filename: str) -> str:
    pdf_buffer.seek(0)
    file_drive = drive.CreateFile({
        "title": filename,
        "parents": [{"id": DRIVE_FOLDER_ID}]
    })
    file_drive.SetContentString(pdf_buffer.getvalue().decode("latin1"))
    file_drive.Upload()
...
def get_ai_recommendations(status, age_m, wfh_p, hfa_p, bmi):
    ...
    if status in ["Obese","Overweight"]:
        recs += ["- Balanced meals with vegetables, fruits, lean proteins.","- Avoid sugary drinks and snacks.","- 60+ mins daily activity.","- Pediatric consultation if BMI>30."]
    elif status=="Underweight":
        recs += ["- Increase nutrient-dense foods (nuts, dairy, eggs).","- Frequent small meals.","- Monitor growth monthly."]
    elif status=="Stunted":
        recs += ["- Focus on iron, zinc, vitamin A-rich foods.","- Adequate protein intake.","- Pediatric evaluation recommended."]
    else:
        recs += ["- Continue balanced diet.","- Encourage 60+ mins active play.","- Regular pediatric check-ups."]
    return recs
...
def generate_report(age_m, ht, wt, sex, model, scaler):
    ...
    bmi = wt / ((ht/100)**2)
    who_msgs=[]
    who_msgs.append((f"Wt-for-height: P{wfh_p:.1f}" if 3<=wfh_p<=85 else "Risk!", colors.red if wfh_p<3 or wfh_p>85 else colors.green))
    who_msgs.append((f"Ht-for-age: P{hfa_p:.1f}" if hfa_p>=3 else "Risk!", colors.red if hfa_p<3 else colors.green))
...
def create_pdf_report(child_name, age_months, report):
    ...
    c.setFont("Helvetica-Bold",14); c.drawString(3*cm,height-7*cm,"WHO Assessment:")
    c.setFont("Helvetica",12); y=height-7.7*cm
    for msg,color in report['who_msgs']: c.setFillColor(color); c.drawString(4*cm,y,msg); y-=0.7*cm
    c.setFillColor(colors.black)
    c.setFont("Helvetica-Bold",14); c.drawString(3*cm,y-0.3*cm,"AI Recommendations:"); c.setFont("Helvetica",12); y-=1*cm
    for rec in report['recommendations']: c.drawString(4*cm,y,rec.replace("**","")); y-=0.7*cm; 
    if y<5*cm: c.showPage(); y=height-3*cm

    plt.figure(figsize=(6,4))
    hfa_x=list(report['hfa_curve'].keys()); hfa_y=list(report['hfa_curve'].values())
    wfh_x=list(report['wfh_curve'].keys()); wfh_y=list(report['wfh_curve'].values())
    plt.plot(hfa_x,hfa_y,label='Height-for-age',color='green'); plt.scatter([report['ht']],[report['ht']],color='blue',label='Child Height')
    plt.xlabel("Percentile"); plt.ylabel("Height (cm)"); plt.title("Height-for-Age"); plt.legend(); plt.tight_layout(); plt.savefig("hfa_chart.png"); plt.close()
    plt.figure(figsize=(6,4))
    plt.plot(wfh_x,wfh_y,label='Weight-for-height',color='orange'); plt.scatter([report['ht']],[report['wt']],color='red',label='Child Weight')
    plt.xlabel("Percentile"); plt.ylabel("Weight (kg)"); plt.title("Weight-for-Height"); plt.legend(); plt.tight_layout(); plt.savefig("wfh_chart.png"); plt.close()

    c.showPage(); c.drawImage("hfa_chart.png",2*cm,height/2,width=16*cm,height=9*cm)
    c.drawImage("wfh_chart.png",2*cm,2*cm,width=16*cm,height=9*cm)
    c.showPage(); c.save(); buffer.seek(0)
    return buffer

# ------------------- STREAMLIT INTERFACE -------------------
st.title("🧒 Child Growth Advisor with PDF & Drive Upload")
...
with st.sidebar:
    ...
    age_months = st.number_input("Age in Months",0,60,24,1)
    height_cm = st.number_input("Height (cm)",40.0,130.0,85.0,0.1,format="%.1f")
    weight_kg = st.number_input("Weight (kg)",1.0,40.0,12.0,0.1,format="%.1f")
...
if generate_button and growth_model and scaler:
    ...
    if report:
        ...
        with col_who: st.subheader("📈 WHO Assessment"); 
        for msg,color in report['who_msgs']: st.markdown(f"- {msg}")
        with col_ai: st.subheader("🤖 AI Recommendations")
        ...
        st.download_button("📄 Download PDF Report",data=pdf_buffer,file_name=f"{child_name.replace(' ','_')}_Growth_Report.pdf",mime="application/pdf")
        # --- Upload to Google Drive ---
        drive_link = upload_pdf_to_drive(pdf_buffer,f"{child_name}_Growth_Report.pdf")
        st.markdown(f"✅ PDF uploaded to Google Drive: [Open PDF]({drive_link})")
