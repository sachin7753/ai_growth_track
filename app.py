...
DRIVE_FOLDER_ID = "1u9uDdhJ0Q8GolChkIk6-otOi6pYggTxP"  # Your Google Drive folder ID
SERVICE_ACCOUNT_FILE = "service_account.json"  # Your JSON credentials
...
def generate_report(age_m: int, ht: float, wt: float, sex: str, model: GrowthNet, scaler) -> dict:
    ...
    bmi = wt / ((ht / 100) ** 2)
    who_msgs = []
    who_msgs.append((f"Wasting risk (Weight-for-height at P{wfh_p:.1f})", colors.red) if wfh_p < 3 else ("Weight-for-height is in a healthy range.", colors.green))
    who_msgs.append((f"Possible overweight risk (Weight-for-height at P{wfh_p:.1f})", colors.red) if wfh_p > 85 else None)
    who_msgs.append((f"Stunting risk (Height-for-age at P{hfa_p:.1f})", colors.red) if hfa_p < 3 else ("Height-for-age is in a healthy range.", colors.green))
    recommendations = get_ai_recommendations(ai_status, age_m, wfh_p, hfa_p, bmi)
    return {"wfh_p": wfh_p, "hfa_p": hfa_p, "bmi": bmi, "who_msgs": [m for m in who_msgs if m], "recommendations": recommendations, "ai_status": ai_status, "confidence": confidence, "hfa_curve": hfa_curve, "wfh_curve": wfh_curve, "age_d": age_d, "ht": ht, "wt": wt}
...
def create_pdf_report(child_name: str, age_months: int, report: dict) -> BytesIO:
    ...
    c.setFont("Helvetica-Bold", 14); c.drawString(3*cm, y-0.3*cm, "AI Recommendations:")
    c.setFont("Helvetica", 12); y -= 1*cm
    ...
    plt.xlabel("Percentile"); plt.ylabel("Height (cm)"); plt.title("Height-for-Age Percentile"); plt.legend(); plt.tight_layout(); plt.savefig("hfa_chart.png"); plt.close()
    plt.figure(figsize=(6,4))
    ...
    plt.xlabel("Percentile"); plt.ylabel("Weight (kg)"); plt.title("Weight-for-Height Percentile"); plt.legend(); plt.tight_layout(); plt.savefig("wfh_chart.png"); plt.close()
...
# ------------------- STREAMLIT INTERFACE -------------------
st.title("🧒 Hybrid AI Child Growth Advisor with PDF Charts & Google Drive")
...
if generate_button and growth_model and scaler:
    ...
    if report:
        ...
        # Upload to Google Drive and show link
        try:
            drive_link = upload_pdf_to_drive(pdf_buffer, f"{child_name.replace(' ', '_')}_Growth_Report.pdf")
            st.success(f"PDF uploaded to Google Drive! [Open PDF]({drive_link})")
        except Exception as e:
            st.error(f"Failed to upload PDF to Drive: {e}")
elif not (growth_model and scaler):
    st.warning("Cannot generate report because AI model or scaler not loaded.")
